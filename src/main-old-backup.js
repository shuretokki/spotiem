...
// DEPRECATED: Old product card function - replaced by products.js module
function createProductCard_OLD(
  product,
  isDesktopOnly = false,
...
// Track current displayed products count
let currentDisplayCount = 8;
let currentCategory = 'All';

// Render Discover Section (8 cards + 1 mobile-centered) with flex-wrap
function renderDiscoverSection(category = 'All', append = false) {
  ...
  if (!append) {
    // Reset count when changing category
    currentDisplayCount = 8;

    // Add fade-out animation
    discoverGrid.style.opacity = '0';
    discoverGrid.style.transform = 'translateY(10px)';

    setTimeout(() => {
      // Clear all cards
      discoverGrid.innerHTML = '';

      // Render initial 8 products (no 9th card initially to make space for "See more")
      ...
      discoverGrid.style.opacity = '1';
      discoverGrid.style.transform = 'translateY(0)';

      // Show/hide "See more" button
      updateSeeMoreButton(products.length);
    }, 200);
  } else {
    // Append more products (load 6 more at a time)
    ...
    newProducts.forEach((product, index) => {
      const card = createDiscoverCard(product, false);
    ...
    currentDisplayCount += newProducts.length;
    updateSeeMoreButton(products.length);
  }
}
...
function updateSeeMoreButton(totalProducts) {
  ...
  // Show button only for "All" category and if there are more products to show
  if (currentCategory === 'All' && currentDisplayCount < totalProducts) {
...
function setupCategoryFilters() {
  ...
  filterButtons.forEach((button) => {
    button.addEventListener('click', () => {
      ...
      // Prevent re-filtering same category
      ...
      filterButtons.forEach((btn) => {
        ...
        if (btn === button) {
          ...
          btn.classList.add('bg-[#57B660]', 'text-black', 'font-bold');
        } else {
          // Unselected state
          btn.classList.remove('bg-[#57B660]', 'text-black', 'font-bold');
          btn.classList.add(
            'bg-transparent',
            'border',
            'border-gray-600',
      ...
      // Re-render discover section with filtered products
      renderDiscoverSection(category, false);
    });
  });
  ...
  if (seeMoreBtn) {
    seeMoreBtn.addEventListener('click', () => {
      renderDiscoverSection(currentCategory, true);
    });
  }
}
